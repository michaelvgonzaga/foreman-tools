...
| M7 — tag-exists subcommand | Pre-flight tag check in one call | `foreman-tools tag-exists <path> <version>` returns `{"exists": bool, "tag": "string"}`; `/release` and `/brew-release` use it in Step 2 pre-flight instead of `git tag \| grep` |
